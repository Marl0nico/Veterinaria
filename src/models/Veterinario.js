...
const veterinarioSchema=new Schema({
    nombre:{
        type: String,
        require: true, //debe ser requerido y no debe quedar en nulo
        trim: true //significa que debe quitar espacios tanto de adelanta como atrás
    },
    apellido:{
        type: String,
        require: true,
    ...
    direccion:{
        type: String,
        require: true,
        default: null //agrega el valor null en caso que no se agregue nada
    },
    telefono:{
        type: Number,
        trim: true,
...
{
    timestamps: true //creación y actualización de registros
})
//método para cifrar la contraseña del veterinario
veterinarioSchema.methods.encrypPassword= async function(password){
    const salt=await bcrypt.genSalt(10)
    const passwordEncryp=await bcrypt.hash(password, salt)
    return passwordEncryp
...
veterinarioSchema.methods.matchPassword=async function(){
    const response=await bcrypt.compare(password, this.password)
    return response
}
